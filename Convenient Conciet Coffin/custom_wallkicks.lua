...
local SURFACE_FIRSTY = 0x2
local SURFACE_WIDE_ANGLE = 0x6
...
local function on_set_mario_action(m)
    ...
    if wall and m.action == ACT_AIR_HIT_WALL then
        if wall.type == SURFACE_FIRSTY then
...
---@param m MarioState
local function wallkicks(m)
    if m.playerIndex ~= 0 then return end
    local wall = m.wall

    if not wall then return end
    if wall.type ~= SURFACE_WIDE_ANGLE then return end
